...
public class DaoAluno {
    ...
    public static Aluno localizarAlunoPorId(int id){
        Session s = ConexaoBD.getSessionFactory().openSession();
        s.beginTransaction();
    ...
    public static Aluno localizarAlunoPorRg(String rg){
        Session s = ConexaoBD.getSessionFactory().openSession();
        s.beginTransaction();
        Aluno lista = (Aluno) s.createQuery("FROM Aluno WHERE rg = " + rg).uniqueResult();
        s.getTransaction().commit();
        s.close();
        return lista;
    }
